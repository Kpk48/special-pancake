...
print(f"Total Test Samples: {len(gt1_decontam)}")
print(f"Stage 1 Acc: {c1.sum()}/{len(c1)} = {c1.mean()*100:.4f}% ({c1.mean()*100:.2f}%)")
print(f"Stage 2 Acc: {c2.sum()}/{len(c2)} = {c2.mean()*100:.4f}% ({c2.mean()*100:.2f}%)")
print(f"Stage 3 Acc: {c3.sum()}/{len(c3)} = {c3.mean()*100:.4f}% ({c3.mean()*100:.2f}%)")
print(f"Joint 3-Way Acc: {joint.sum()}/{len(joint)} = {joint.mean()*100:.4f}% ({joint.mean()*100:.2f}%)")

print(f"Number of samples where Stage 3 is correct but Stage 1 is wrong: {((~c1) & c3).sum()}")
print(f"Number of samples where Stage 3 is correct but Stage 2 is wrong: {((~c2) & c3).sum()}")
